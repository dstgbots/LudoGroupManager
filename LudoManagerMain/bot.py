...
class LudoBotManager:
        ...
        # NOTE: Pyrogram handlers removed - test.py handles message detection
        ...
        async def _initialize_pyrogram_properly(self):
            ...
            try:
                ...
                # NOTE: Handlers are now in test.py - no local handlers needed
                logger.info("✅ Pyrogram client ready for integration with test.py")
                ...
                # NOTE: Handler verification removed - handlers are in test.py
    ...
    """
        ...
        async def run_async(self):
            ...
            try:
                ...
                # NOTE: Pyrogram initialization removed - test.py handles Pyrogram client
                logger.info("ℹ️ Pyrogram client setup delegated to test.py")
        ...
        # Integration Functions for test.py to call
        def handle_new_game_from_pyrogram(self, game_data, message_id=None, admin_id=None):
            """
            Handle new game creation from test.py Pyrogram detection.
            Integrates with existing game management system.
            """
            try:
                logger.info("🎮 NEW GAME DETECTED VIA PYROGRAM INTEGRATION")
                logger.info(f"👥 Players: {', '.join(game_data['players'])}")
                logger.info(f"💰 Amount: {game_data['amount']}")
                logger.info(f"📨 Message ID: {message_id}")
                
                # Store in active games using existing structure
                if message_id:
                    self.active_games[str(message_id)] = game_data
                    logger.info(f"🎮 Game stored in active_games with ID: {message_id}")
                
                # Store in database using existing database methods
                try:
                    game_document = {
                    ...
                    result = self.games_collection.insert_one(game_document)
                ...
                # Log admin action
                if admin_id:
                    logger.info(f"👤 Created by admin: {admin_id}")
                    
                logger.info("✅ New game processed successfully via Pyrogram integration")
                
            except Exception as e:
                logger.error(f"❌ Error handling new game from Pyrogram: {e}")

        def handle_winner_from_pyrogram(self, game_data, winner, message_id=None, admin_id=None):
            """
            Handle winner detection from test.py Pyrogram detection.
            Integrates with existing winner processing system.
            """
            try:
                logger.info("🏆 WINNER DETECTED VIA PYROGRAM INTEGRATION")
                ...
                if message_id:
                    message_id_str = str(message_id)
                    if message_id_str in self.active_games:
                        del self.active_games[message_id_str]
                        logger.info(f"🗑️ Removed game from active_games: {message_id}")
                ...
                try:
                    ...
                    result = self.games_collection.update_one(query, update_data)
                    if result.modified_count > 0:
                        logger.info(f"✅ Game updated in database with winner: {winner}")
                    else:
                        logger.warning(f"⚠️ No matching active game found in database")
                        
                except Exception as e:
                    logger.error(f"❌ Error updating game winner in database: {e}")
                
                # Process winner balance
                try:
                    winner_user = self.users_collection.find_one({"username": winner})
                    if winner_user:
                        # Get current balance
                        current_balance = winner_user.get("balance", 0.0)
                        new_balance = current_balance + game_data['amount']
                        ...
                        transaction = {
                            ...
                            "description": f"Won game with prize ₹{game_data['amount']}",
                            "timestamp": datetime.now()
                        }
                        self.transactions_collection.insert_one(transaction)
                        
                        logger.info(f"💰 Added ₹{game_data['amount']} to {winner}'s balance")
                    else:
                        logger.info(f"ℹ️ Winner {winner} not found in user database - no balance update")
                        
                except Exception as e:
                    logger.error(f"❌ Error processing winner balance: {e}")
                
                # Send winner announcement to group
                try:
                    winner_message = f"🎉 **WINNER ANNOUNCED!** 🎉\n\n"
                    winner_message += f"🏆 **Winner:** @{winner}\n"
                    winner_message += f"💰 **Prize:** ₹{game_data['amount']}\n"
                    winner_message += f"👥 **Players:** {', '.join(['@' + p for p in game_data['players']])}\n"
                    winner_message += f"📅 **Game Date:** {game_data['created_at'].strftime('%Y-%m-%d %H:%M') if game_data.get('created_at') else 'N/A'}\n\n"
                    winner_message += "🎮 Congratulations to the winner! 🎊"
                    
                    # Send using Pyrogram if available
                    async def send_announcement():
                        try:
                            if self.pyro_client:
                                ...
                                logger.warning("⚠️ Pyrogram client not available for announcement")
                        except Exception as e:
                    ...
                    logger.error(f"❌ Error preparing winner announcement: {e}")
                
                # Log admin action
                if admin_id:
                    logger.info(f"👤 Winner declared by admin: {admin_id}")
                    
                logger.info("✅ Winner processed successfully via Pyrogram integration")
                
            except Exception as e:
                logger.error(f"❌ Error handling winner from Pyrogram: {e}")

# Global instance for easy access from test.py
bot_manager = None

def initialize_bot_manager():
    """Initialize the global bot manager instance"""
    global bot_manager
    if bot_manager is None:
        bot_manager = LudoBotManager()
    return bot_manager

def handle_new_game(game_data, message_id=None, admin_id=None):
    """Wrapper function for test.py to call when new game is detected"""
    global bot_manager
    if bot_manager is None:
        bot_manager = initialize_bot_manager()
    bot_manager.handle_new_game_from_pyrogram(game_data, message_id, admin_id)

def handle_winner(game_data, winner, message_id=None, admin_id=None):
    """Wrapper function for test.py to call when winner is detected"""
    global bot_manager
    if bot_manager is None:
        bot_manager = initialize_bot_manager()
    bot_manager.handle_winner_from_pyrogram(game_data, winner, message_id, admin_id)
